...
$(document).ready(function () {
  ...
  $('.add-value').click(function () {
    // TODO: Add a remove button as a sibling
    var input = '<input type="text" class="input" name="value-names[]" value="" placeholder="New Value..." required>';
    var button = '<button type="button" class="button is-danger remove-value">Remove Value</button>';
  ...
  $('.button[name=add-or]').click(function () {
    ...
    if (label.children('.bound-group').length) {
      var newBoundGroup = label.children('.bound-group').last().clone();
      var oldID = newBoundGroup.attr('id');
      var regex = /bound-[0-9]/;
      var count = Number(oldID.slice(-1)) + 1;
      var newID = oldID.substring(0, oldID.length - 1) + count;
      newBoundGroup.attr('id', newID);
      newBoundGroup.children('input').each(function () {
        var oldName = $(this).attr('name');
        var newName = oldName.replace(oldID, newID);
        $(this).attr('name', newName);
      });
      newBoundGroup.insertAfter(label.children('.bound-group').last());
    } else {
      var newInput = label.children('input').last().clone();
  ...
  $('.button[name=remove-or]').click(function () {
    ...
    if (label.children('.bound-group').length > 1) {
      var newBoundGroup = label.children('.bound-group').last().remove();
    } else if (label.children('input').length > 1) {
      var newInput = label.children('input').last().remove();
    }
  });
  $('th').click(function () {
    var form = $('.form');
    ...
    var sortDirection = $('.form').find('#sort-direction').val();
    var flippedDirection = sortDirection == 'ASC' ? 'DESC' : 'ASC';
  ...
  fixTableStripes(); // Fix Table Striping while searching or sorting
...
__webpack_require__(/*! /home/chrisvm/Desktop/code/NUF-Inventory/resources/js/app.js */"./resources/js/app.js");
module.exports = __webpack_require__(/*! /home/chrisvm/Desktop/code/NUF-Inventory/resources/sass/app.scss */"./resources/sass/app.scss");
